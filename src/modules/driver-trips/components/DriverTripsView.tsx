...
export function DriverTripsView() {
  ...
  const resetForm = () => {
    ...
    setDepartureDate(todayISO()); setAgreedFreight(''); setInitialPaid(''); setTeaMoney(''); setNote(''); setErr('');
    setAdding(false);
  };
  ...
  const all = trips ?? [];
  // open = not arrived yet OR arrived but unpaid delay
  const isOpen = (t: typeof all[0]) => !t.arrivalDate || (t.remainingDelay ?? 0) > 0;
    ...
    <>
      ...
      <div style={{ display: 'grid', gap: 10 }}>
        {searched.map((trip) => {
          ...
          const borderColor = isTrulyClosed ? 'var(--credit)' : hasUnpaidDelay ? '#e67e00' : 'var(--debit)';
          return (
            <div
              key={trip.id}
              className="card"
